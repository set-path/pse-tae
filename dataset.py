...
class PixelSetData(data.Dataset):
    def __init__(self, folder, labels, npixel, sub_classes=None, norm=None,
        ...
        self.pid = [int(f.split('.')[0]) for f in os.listdir(self.data_folder)]
        self.pid = list(np.sort(self.pid))

        self.pid = list(map(str, self.pid))
        self.len = len(self.pid)

        if os.path.exists(os.path.join(self.meta_folder, 'labels.json')):
            # Get Labels
            if sub_classes is not None:
                sub_indices = []
                convert = dict((c, i) for i, c in enumerate(sub_classes))

            with open(os.path.join(self.meta_folder, 'labels.json'), 'r') as file:
            ...
            self.target = [-1] * len(self.pid)

        with open(os.path.join('data', 'META', 'dates.json'), 'r') as file:
            d = json.loads(file.read())
        self.dates = [d[str(i)] for i in range(len(d))]
        self.date_positions = date_positions(self.dates)

        if self.extra_feature is not None:
            with open(os.path.join('data', 'META', '{}.json'.format(extra_feature)), 'r') as file:
            ...
            self.extra = json.load(open(os.path.join(self.meta_folder, '{}.json'.format(extra_feature)), 'r'))
    ...
    def __getitem__(self, item):
        ...
        x0 = np.load(os.path.join(self.data_folder, '{}.npy'.format(self.pid[item])))
        y = self.target[item]
